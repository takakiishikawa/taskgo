...
export async function POST(request: Request) {
  try {
    ...
    const [{ data: tasks }, { data: layers }] = await Promise.all([
      db.from('tasks').select('title, layer_type, status')
        .eq('user_id', user.id).neq('status', 'done').order('updated_at', { ascending: false }).limit(20),
      db.from('design_layers').select('layer_type, title, cover_until')
...
各課題は課題名・なぜ重要か・推奨アクションの3点で記述してください。`
    ...
    const { data: saved, error } = await db.from('ai_suggestions').insert({
      task_id: taskId,
      user_id: user.id,
