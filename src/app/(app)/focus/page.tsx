...
import {
  getWeeklyFocusTasks, addTaskToWeeklyFocus, removeTaskFromWeeklyFocus,
  markWeeklyFocusTaskDone, getTasks, type WeeklyFocusTaskWithTask,
} from '@/lib/db'
...
export default function FocusPage() {
  ...
  useEffect(() => { WEEK_OFFSETS.forEach((o) => loadWeek(o)) }, [loadWeek])
  ...
  const handleRemove = async (wfTaskId: string) => {
    try {
      ...
      setFocusByWeek((prev) => ({ ...prev, [activeTab]: (prev[activeTab] ?? []).filter((wf) => wf.id !== wfTaskId) }))
    } catch {
  ...
  const handleToggleDone = async (wfTask: WeeklyFocusTaskWithTask) => {
    try {
      ...
      setFocusByWeek((prev) => ({
        ...prev,
        [activeTab]: (prev[activeTab] ?? []).map((wf) => wf.id === wfTask.id ? { ...wf, is_done: !wf.is_done } : wf),
      }))
    } catch {
  ...
  const filteredTasks = allTasks.filter(
    (t) => !alreadyAddedIds.has(t.id) && (taskSearch === '' || t.title.toLowerCase().includes(taskSearch.toLowerCase()))
  )
    ...
    <div className="px-8 py-8 max-w-3xl">
          ...
          return (
            <button
              ...
              className={`relative flex items-center gap-1.5 px-4 py-2.5 text-sm font-medium transition-colors -mb-px border-b-2 ${
                isActive ? 'text-foreground border-[color:var(--color-primary)]' : 'text-muted-foreground border-transparent hover:text-foreground'
              }`}
            >
              {formatWeekLabel(offset)}
              {count > 0 && (
                <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${allDone ? 'bg-success-subtle text-success' : 'bg-accent text-muted-foreground'}`}>
                  {doneCount}/{count}
                </span>
              )}
            </button>
          ...
          currentFocus.map((wfTask, i) => (
            ...
            >
              ...
              <Link href={`/tasks/${wfTask.task_id}`} className="flex-1 min-w-0">
                <span className={`text-sm block truncate hover:text-primary transition-colors ${wfTask.is_done ? 'opacity-40' : ''}`}>
                  {wfTask.task.title}
                </span>
              </Link>
              ...
              <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                <Link href={`/tasks/${wfTask.task_id}`} className="p-1 rounded text-muted-foreground hover:text-foreground transition-colors">
                  <ChevronRight className="w-3 h-3" />
                </Link>
                <button onClick={() => handleRemove(wfTask.id)} className="p-1 rounded text-muted-foreground hover:text-destructive transition-colors">
                  <X className="w-3 h-3" />
                </button>
              </div>
            </div>
      ...
      <Dialog open={addOpen} onOpenChange={setAddOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle className="text-sm">{formatWeekLabel(activeTab)}のフォーカスにタスクを追加</DialogTitle>
          </DialogHeader>
          <div className="mt-2">
            <Input
              value={taskSearch}
