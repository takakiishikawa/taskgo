...
import type {
  Task, DesignLayer, AiSuggestion, LayerType, TaskStatus,
  Tag, WeeklyFocus, WeeklyFocusTask, WeeklySummary, RecurringTask, RecurringTaskType,
} from '@/types/database'
...
export async function updateTask(
  id: string,
  updates: Partial<Pick<Task, 'title' | 'description' | 'layer_type' | 'status' | 'is_focus' | 'due_date' | 'output_note' | 'completed_at'>>
): Promise<Task> {
...
export async function getFocusTasks(): Promise<Task[]> {
  const { data, error } = await db().from('tasks').select('*')
    .eq('is_focus', true).neq('status', 'done')
...
export async function getTagsForTasks(taskIds: string[]): Promise<Record<string, Tag[]>> {
  ...
  const { data: taskTagData } = await db().from('task_tags').select('task_id, tag_id').in('task_id', taskIds)
  if (!taskTagData?.length) return {}
  const tagIds = [...new Set(taskTagData.map((tt) => tt.tag_id))]
  const { data: tagsData } = await db().from('tags').select('*').in('id', tagIds)
...
export async function getTaskTagIds(taskId: string): Promise<string[]> {
  const { data } = await db().from('task_tags').select('tag_id').eq('task_id', taskId)
...
export async function addTagToTask(taskId: string, tagName: string): Promise<Tag> {
  ...
  const { data: existing } = await db().from('tags').select('*')
    .eq('user_id', user.id).eq('name', tagName.trim()).maybeSingle()
  ...
  if (existing) {
    ...
    const { data: created, error } = await db().from('tags')
      .insert({ user_id: user.id, name: tagName.trim() }).select().single()
    if (error) throw new Error(`addTag: ${error.message}`)
    tag = created
  }

  // task_tags に追加（重複チェック）
  const { data: existingLink } = await db().from('task_tags')
    .select('id').eq('task_id', taskId).eq('tag_id', tag.id).maybeSingle()
  if (!existingLink) {
...
export async function removeTagFromTask(taskId: string, tagId: string): Promise<void> {
  const { error } = await db().from('task_tags').delete()
    .eq('task_id', taskId).eq('tag_id', tagId)
...
export async function getDesignLayers(layerType?: 'core_value' | 'roadmap' | 'spec_design'): Promise<DesignLayer[]> {
  let query = db().from('design_layers').select('*').order('last_updated_at', { ascending: false })
...
export async function upsertDesignLayer(params: {
  ...
  if (params.id) {
    const { data, error } = await db().from('design_layers').update({
      title: params.title, content: params.content ?? null,
      cover_until: params.cover_until ?? null, last_updated_at: now,
    }).eq('id', params.id).select().single()
    if (error) throw new Error(`updateDesignLayer: ${error.message}`)
    return data
  } else {
    const { data, error } = await db().from('design_layers').insert({
      user_id: user.id, layer_type: params.layer_type, title: params.title,
      content: params.content ?? null, cover_until: params.cover_until ?? null, last_updated_at: now,
    }).select().single()
...
export async function getLatestDesignLayer(layerType: 'core_value' | 'roadmap' | 'spec_design'): Promise<DesignLayer | null> {
  const { data } = await db().from('design_layers').select('*').eq('layer_type', layerType)
    .order('cover_until', { ascending: false, nullsFirst: false }).limit(1).maybeSingle()
  return data
}
...
export async function getAiSuggestions(taskId: string): Promise<AiSuggestion[]> {
  const { data, error } = await db().from('ai_suggestions').select('*')
...
export async function getOrCreateWeeklyFocus(weekStart: string): Promise<WeeklyFocus> {
  ...
  const { data, error } = await db().from('weekly_focuses')
    .insert({ user_id: user.id, week_start: weekStart }).select().single()
...
export async function getWeeklyFocusTasks(weekStart: string): Promise<WeeklyFocusTaskWithTask[]> {
  const focus = await getWeeklyFocus(weekStart)
  if (!focus) return []

  const { data: wfTasks } = await db().from('weekly_focus_tasks').select('*')
    .eq('weekly_focus_id', focus.id).order('created_at')
...
export async function addTaskToWeeklyFocus(weekStart: string, taskId: string): Promise<void> {
  const focus = await getOrCreateWeeklyFocus(weekStart)
  // 重複チェック
  const { data: existing } = await db().from('weekly_focus_tasks').select('id')
    .eq('weekly_focus_id', focus.id).eq('task_id', taskId).maybeSingle()
  if (existing) return
  const { error } = await db().from('weekly_focus_tasks')
    .insert({ weekly_focus_id: focus.id, task_id: taskId, is_done: false })
  if (error) throw new Error(`addTaskToWeeklyFocus: ${error.message}`)
}

export async function removeTaskFromWeeklyFocus(wfTaskId: string): Promise<void> {
  const { error } = await db().from('weekly_focus_tasks').delete().eq('id', wfTaskId)
  if (error) throw new Error(`removeTaskFromWeeklyFocus: ${error.message}`)
}

export async function markWeeklyFocusTaskDone(wfTaskId: string, isDone: boolean): Promise<void> {
  const { error } = await db().from('weekly_focus_tasks').update({ is_done: isDone }).eq('id', wfTaskId)
...
export async function initRecurringTasksIfNeeded(): Promise<void> {
  ...
  await db().from('recurring_tasks').insert([
    { user_id: user.id, task_type: 'issue_discovery_short', next_generate_at: today },
    { user_id: user.id, task_type: 'issue_discovery_mid', next_generate_at: today },
  ])
}
...
export async function checkAndGenerateRecurringTasks(): Promise<Task[]> {
  ...
  for (const rt of due) {
    ...
    await db().from('recurring_tasks').update({ next_generate_at: toYMD(nextDate) }).eq('id', rt.id)

    // タスク生成
    const { data: task, error } = await db().from('tasks').insert({
      user_id: user.id,
      title: titleMap[rt.task_type],
      description: null, layer_type: 'other',
      status: 'pending', is_focus: false,
    }).select().single()
